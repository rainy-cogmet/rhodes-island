#!/bin/bash
# ══════════════════════════════════════════════════════════════
# 罗德岛体系 · OpenClaw Multi-Agent System 一键安装脚本
# ══════════════════════════════════════════════════════════════
set -e
...
banner() {
  echo ""
  echo -e "${BLUE}╔══════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║  🐰  罗德岛体系 · OpenClaw Multi-Agent    ║${NC}"
  echo -e "${BLUE}║       安装向导                            ║${NC}"
  echo -e "${BLUE}╚══════════════════════════════════════════╝${NC}"
  echo ""
}
...
backup_existing() {
  AGENTS_DIR="$OC_HOME"
  BACKUP_DIR="$OC_HOME/backups/pre-rhodes-$(date +%Y%m%d-%H%M%S)"
  HAS_EXISTING=false
...
create_workspaces() {
  ...
  # 罗德岛角色列表
  AGENTS=(amiya theresis kaltsit Mon3tr Closure Silence Logos Saria Texas warfarin Exusiai)
...
register_agents() {
  ...
  # 备份配置
  cp "$OC_CFG" "$OC_CFG.bak.rhodes-$(date +%Y%m%d-%H%M%S)"
...
# 罗德岛角色权限矩阵
AGENTS = [
  {"id": "amiya",    "subagents": {"allowAgents": ["theresis"]}},
  {"id": "theresis", "subagents": {"allowAgents": ["kaltsit", "Mon3tr"]}},
  {"id": "kaltsit",  "subagents": {"allowAgents": ["Mon3tr", "theresis"]}},
  {"id": "Mon3tr",   "subagents": {"allowAgents": ["theresis", "kaltsit", "Closure", "Silence", "Logos", "Saria", "Texas", "warfarin"]}},
  {"id": "Closure",   "subagents": {"allowAgents": ["Mon3tr"]}},
  {"id": "Silence",   "subagents": {"allowAgents": ["Mon3tr"]}},
  {"id": "Logos",     "subagents": {"allowAgents": ["Mon3tr"]}},
  {"id": "Saria",     "subagents": {"allowAgents": ["Mon3tr"]}},
  {"id": "Texas",     "subagents": {"allowAgents": ["Mon3tr"]}},
  {"id": "warfarin",  "subagents": {"allowAgents": ["Mon3tr"]}},
  {"id": "Exusiai",   "subagents": {"allowAgents": []}},
]
...
tasks = [
    {
        ...
        "ac": "系统正常运行",
        "flow_log": [
            {"at": "2024-01-01T00:00:00Z", "from": "博士", "to": "阿米娅", "remark": "初始化罗德岛体系"},
            {"at": "2024-01-01T00:01:00Z", "from": "阿米娅", "to": "特蕾西娅", "remark": "规划方案提交审核"},
            {"at": "2024-01-01T00:02:00Z", "from": "特蕾西娅", "to": "凯尔希", "remark": "✅ 准奏"},
            {"at": "2024-01-01T00:03:00Z", "from": "凯尔希", "to": "Mon3tr", "remark": "派发：系统初始化"},
            {"at": "2024-01-01T00:04:00Z", "from": "Mon3tr", "to": "阿米娅", "remark": "✅ 完成"},
        ]
    }
]
